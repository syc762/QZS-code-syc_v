...
class tf:
    def __init__(self):
        ##### *!* Channels
        self.channel = [1,2] ### original: str(1) ### 
...
def plot_tf_from_df(df, filename, save_dir):
    
    x = df.iloc[:,0].to_numpy()
    
    channel1 = df.iloc[:,1].to_numpy() 
    channel2 = df.iloc[:,3].to_numpy()
    normalized = df.iloc[:,5].to_numpy()
...
numPoints = 100
volt = '0.6' # Must be lower than 600mV!!! 
# Run label
springType = "stiffestSprings"
label = "stiffestSprings_1lightmass_flexureAndPlateAndDampers_" + str(numPoints) + "points_" + volt+"V"
...
frequency = np.logspace(0, 1.67, num=numPoints) # Up to 50Hz it's ok
...
tf = tf()
tf.open_instruments()
tf.initialize_instruments(voltage=volt)
