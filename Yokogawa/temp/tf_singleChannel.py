...
def plot(df, filename, save_dir, timestamp, label): # Need to change which column it's referring to based on the old excel plots
    ...
    plt.plot(x, V_signal, label="$V_{in}$", c=color, marker='.', linestyle='dashed')
    plt.loglog()
    ...
    try:
        plt.savefig(os.path.join(save_dir, timestamp + "_" + filename + "_" + label + ".png"), transparent=True, bbox_inches='tight')
    except Exception as e:
        print(e)
    plt.close()
...
if __name__ == "__main__":
    ...
    for v in volt:
        ...
        df = save_data_to_csv([frequency, means1], label, save_dir, timestamp, data_type)

        ### Directly use the data to obtain the transfer function
        plot(df, label, save_dir, timestamp, data_type)
